...
def parse_detail_page(html, url, source):
    ...
    if specs_container:
        ...
        for li in specs_container.find_all("li"):
            lbl_el = li.find(class_="label")
            val_el = li.find(class_="input")
...
async def main(max_pages, limit_per_page, csv_path, json_path, country):
        ...
        try:
            ...
            for page_num in range(1, max_pages + 1):
                ...
                try:
                    ...
                    await bypass_turnstile_if_present(page, target_selector="a[href*='/boats-for-sale/']")
        ...
        for idx, url in enumerate(links_to_scrape):
            ...
            for attempt in range(1, max_retries + 1):
                ...
                try:
                    ...
                    try:
                        await page.goto(url, wait_until="commit", timeout=60000)
                        success = await bypass_turnstile_if_present(page, target_selector="#desc, #specs, h1")
                        if not success:
    ...
    if scraped_dataset:
        ...
        for record in final_list:
            ...
            # Tidy up length_loa
            length = record.get("length_loa", "")
            if length:
                record["length_loa"] = format_length(str(length))
                
            # Tidy up beam
            beam = record.get("beam", "")
            if beam:
                record["beam"] = format_length(str(beam))
                
            # Tidy up draft
            draft = record.get("draft", "")
            if draft:
                record["draft"] = format_length(str(draft))
            
            # Format price (e.g. "$6,599.00 CAD")
            price = record.get("price", "")
            if price:
                price_upper = str(price).upper()
                currency = "CAD"  # default for boatdealers.ca
                ...
                if match:
                    ...
                    try:
                        val = float(num_clean)
                        record["price"] = f"${val:,.2f} {currency}"
                    except (ValueError, TypeError):
                        pass
...
if __name__ == "__main__":
    ...
    default_csv = os.path.join(script_dir, "boatdealers_ca_dataset.csv")
    default_json = os.path.join(script_dir, "boatdealers_ca_dataset.json")
